...
def override_eval_profile_root(profile: EvalOutputProfile, output: str) -> EvalOutputProfile:
    ...
    ``<root>/runs/<run_id>/``. Replacing ``profile.root`` wholesale dropped
    ``/runs`` and tripped ``QODER_MANIFEST_PATH_INVALID`` on the documented
    ``verify --output .repo-agent-eval/runs/<id>`` command.
    """
    root = Path(output)
