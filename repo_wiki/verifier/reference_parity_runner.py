"""reference-repo isolated qoder parity rerun.
...
Phase 29 - Task 29.5: reference-repo qoder parity rerun
...
def _get_reference_root() -> Path:
    """Get REFERENCE_REPO_ROOT from environment or use default."""
    env_path = os.environ.get("REFERENCE_REPO_ROOT")
    if env_path:
        return Path(env_path)
    # For testing, use a default path if not set
    # In CI/CD, this should be explicitly set
    return Path("/tmp/reference-mock-root")


def _get_reference_roots() -> tuple[Path, Path, Path]:
    """Get all reference paths, with lazy evaluation."""
    reference_root = _get_reference_root()
    return (
        reference_root,
        reference_root / ".qoder" / "repowiki" / "zh",
        reference_root / ".repo-agent-eval",
    )


REFERENCE_REPO_ROOT, QODER_BASELINE_DIR, REFERENCE_CONTENT_DIR = _get_reference_roots()
...
class ReferenceParityRunner:
    """Runs isolated parity evaluation on reference-repo."""

    def __init__(
        self,
        reference_root: Path = REFERENCE_REPO_ROOT,
        baseline_dir: Path = QODER_BASELINE_DIR,
    ) -> None:
        """Initialize parity runner.

        Args:
            reference_root: reference-repo root
            baseline_dir: Qoder baseline directory
        """
        self.reference_root = Path(reference_root)
        self.baseline_dir = Path(baseline_dir)
        self.run_id = f"reference-parity-{uuid.uuid4().hex[:8]}"

    def get_output_dir(self) -> Path:
        """Get the output directory for this run."""
        return self.reference_root / ".repo-agent-eval" / self.run_id / "content"
...
def run_reference_parity() -> ParityComparisonResult:
    """Run reference-repo parity evaluation.
...
def get_reference_parity_report(run_id: str) -> ParityComparisonResult | None:
    ...
    """
    report_path = REFERENCE_CONTENT_DIR / run_id / "parity_report.json"
