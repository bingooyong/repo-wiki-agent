"""Tests for reference-repo parity runner."""

from pathlib import Path

import pytest

from repo_wiki.verifier.reference_parity_runner import (
    GapItem,
...
class TestReferenceParityRunner:
    """Tests for ReferenceParityRunner."""

    @pytest.fixture
    def setup_reference_structure(self, tmp_path):
        """Set up reference-repo-like directory structure."""
        # Create baseline
...
""")
    ...
    def test_runner_initialization(self, setup_reference_structure):
        """Test runner initializes correctly."""
        runner = ReferenceParityRunner(
            reference_root=setup_reference_structure,
            baseline_dir=setup_reference_structure / ".qoder" / "repowiki" / "zh",
        )
        assert runner.run_id.startswith("reference-parity-")
        assert runner.baseline_dir.exists()

    def test_extract_metrics_from_content(self, setup_reference_structure):
        ...
        content_dir = setup_reference_structure / ".qoder" / "repowiki" / "zh" / "content"
    ...
    def test_run_comparison(self, setup_reference_structure):
    ...
    def test_count_prose(self, setup_reference_structure):
        """Test prose counting."""
        runner = ReferenceParityRunner(reference_root=setup_reference_structure)
...
"""
    ...
    def test_citation_density(self, setup_reference_structure):
    ...
    def test_gap_severity_determination(self, setup_reference_structure):
    ...
    def test_overall_parity_calculation(self, setup_reference_structure):
...
class TestFactoryFunctions:
    ...
    def test_run_reference_parity(self, tmp_path):
        """Test run_reference_parity factory."""
        # Create structure
        baseline = tmp_path / ".qoder" / "repowiki" / "zh" / "content"
        baseline.mkdir(parents=True)
        ...
        runner = ReferenceParityRunner(reference_root=tmp_path)
        result = runner.run_comparison()
