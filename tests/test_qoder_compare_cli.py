...
def test_compare_generates_markdown_and_json_reports(tmp_path: Path) -> None:
    ...
    class _FakeReport:
        def to_dict(self):
            return {
                "metrics": [
                    {"metric_name": "toc_presence", "status": "pass", "measured_value": 1.0, "threshold": 0.8},
                    {"metric_name": "citation_coverage", "status": "pass", "measured_value": 1.0, "threshold": 0.7},
                    {"metric_name": "mermaid_presence", "status": "pass", "measured_value": 0.4, "threshold": 0.3},
                    {"metric_name": "prose_list_ratio", "status": "pass", "measured_value": 0.8, "threshold": 0.4},
                    {"metric_name": "api_aggregation", "status": "pass", "measured_value": 0.7, "threshold": 0.6},
                    {"metric_name": "data_model_aggregation", "status": "pass", "measured_value": 0.7, "threshold": 0.6},
                    {"metric_name": "file_reference_integrity", "status": "pass", "measured_value": 1.0, "threshold": 1.0, "details": {"broken_refs": []}},
                ],
                "summary": {"overall_score": 0.9},
                "blocked": False,
            }

    with patch("repo_wiki.verifier.qoder_parity_metrics.create_parity_report", return_value=_FakeReport()), \
        patch("repo_wiki.verifier.qoder_strict_verifier.verify_qoder_like", return_value={"grade": "PASS"}):
