...
tot = rel = 0
bad = []
_CODE_SUFFIXES = (".py", ".go", ".md", ".yml", ".rst", ".json", ".sql", ".toml")
for p in content.rglob("*.md"):
    ...
    for i, line in enumerate(lines):
        ...
        ids = {
            x
            for x in re.findall(r"`([A-Za-z_][A-Za-z0-9_.]{3,})`", text)
            if "/" not in x and not x.endswith(_CODE_SUFFIXES)
        }
        if STRICT:
        ...
        for f, a, b in cites:
            ...
            if not fp.is_file():
                continue
            src_lines = fp.read_text(errors="replace").split("\n")
            a = int(a)
            b = int(b or a)
            win = "\n".join(src_lines[max(0, a - 4) : b + 3])
            if any(x in win for x in ids):
                hit = True
                break
        if hit:
            rel += 1
            ...
            bad.append(
                {
                    "page": str(p.relative_to(content)),
                    "ids": sorted(ids)[:4],
                    "cites": [f"{f}:{a}-{b or a}" for f, a, b in cites][:2],
                }
            )
print(
    json.dumps(
        {
            "checked": tot,
            "relevant": rel,
            "relevant_pct": round(100 * rel / max(tot, 1), 1),
            "examples": bad[:8],
        },
        ensure_ascii=False,
    )
