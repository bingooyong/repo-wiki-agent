...
class RepoWikiSidebarProvider {
    ...
    renderNoWikiRuns(workspaceRoot) {
...
function loadLlmDisplayInfo(workspaceRoot) {
...
function renderLlmPanel(info, locale) {
    ...
    lines.push(`<div class="llm-row"><span class="llm-key">${fileLabel}</span> <span>${escapeHtml(info.configFile ?? '')}</span></div>`);
    const providerLabel = en ? 'Provider' : '提供商';
    const providerVal = info.provider ?? (en ? '(default anthropic)' : '（默认 anthropic）');
    lines.push(`<div class="llm-row"><span class="llm-key">${providerLabel}</span> <span>${escapeHtml(providerVal)}</span></div>`);
    const primaryModel = info.modelUpdate ??
        info.flatModel ??
        (en ? '(default claude-sonnet-4-5)' : '（默认 claude-sonnet-4-5）');
    const primaryLabel = en ? 'Model (update)' : '更新模型';
    lines.push(`<div class="llm-row"><span class="llm-key">${primaryLabel}</span> <span>${escapeHtml(primaryModel)}</span></div>`);
    if (info.modelInit) {
...
function collectManifestPaths(root, maxDepth) {
    const found = [];
...
function flattenNavigationTree(tree, rootDir) {
    const files = [];
    const seen = new Set();
...
function runTerminalCommand(name, command) {
    const workspaceRoot = getWorkspaceRoot();
...
function renderTreeNode(node, open, pageLabelMap) {
...
function buildPageKey(filePath, absolutePath) {
